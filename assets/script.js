document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeEl) {
        const phrases = ["Safe. Strong. Sustainable.","Built on Engineering Principles.","Designed for Strength.","Engineered to Last.","Strength You Can Trust."];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!audio) {
    console.error("Ambient sound element not found");
    return;
  }
  ...
  const startAudio = () => {
    ...
    audio.play()
      ...
      .catch(err => {
        ...
        // Keep listeners active if play fails, allowing retries on subsequent interactions.
        // You might want to add visual cues to the user here.
        // It's crucial here NOT to remove listeners if play fails,
        // so the next user interaction can try again.
      });
  };

  // Helper to remove all attached listeners
